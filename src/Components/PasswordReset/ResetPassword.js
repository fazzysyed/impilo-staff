...
export default function ResetPassword() {
  ...
  const registrationHandler = async (step) => {
    // error handling
    ...
    if (step == 1) {
      ...
      handleAPIRequest("get", `reset-password-new/${email}`, null)
        .then((response) => {
          if (response.success) {
      ...
      handleAPIRequest("POST", "verify-password-new", {
        email: email,
        code: verifyCode,
      })
        .then(() => {
      ...
      handleAPIRequest("POST", "change-password-new", {
        email: email,
      ...
      <div className="flex w-full h-full justify-center flex-col md:flex-row-reverse ">
        ...
        <div className="h-full w-full flex flex-col justify-center md:w-[90%] md:p-12  overflow-auto p-6 lg:p-24 ">
          ...
          <div className="pt-10 w-full relative">
            ...
            <button
              className="bg-[#524494] text-white w-full py-3 rounded-xl transition-all ease-in-out duration-500 hover:bg-[#0d2041] hover:shadow-lg border hover:border hover:border-[#10274F]"
              onClick={() => registrationHandler(registrationStep)}
            >
              {loading
                ? "Please Wait..."
